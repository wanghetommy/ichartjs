...
1. **Uncommitted release contents.** The complete Iteration 8 and Agent-first release changes are still in the working tree. Review and commit them before tagging.
2. **Missing final tag.** No repository release tag exists. Create the final tag only after the version is changed consistently in `package.json`, `src/index.mjs`, and the changelog.
...
3. Review and commit the release contents.
4. Change `2.0.0-rc.1` to `2.0.0` in package metadata, runtime metadata, and changelog in one change.
5. Re-run all automated, package-consumer, and browser gates.
